...
PREV="<action=playerctl previous><fc=${WHITE2},${FORMATBG}:5><fn=2>玲</fn></fc></action>"
NEXT="<action=playerctl next><fc=${WHITE2},${FORMATBG}:5><fn=2>怜</fn></fc></action>"
	...
	playerctl --player=spotify metadata -f '{{ status }} {{ title }} {{ artist }}' -F |
		while read -r _status ; do
			...
			if [[ -z "${_status}" ]] ; then
				...
				printf "<fc=${FORMATBG},${BACKGROUND}:7><fn=2></fn></fc>%s%s%s %s <fc=${GREY},${FORMATBG}>-</fc> %s<fc=${FORMATBG},${BACKGROUND}:7><fn=2> </fn></fc>\n" \
